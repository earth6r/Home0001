...
import { SanityMediaProps } from '@components/sanity'
...
export interface ImageZoomProps extends HTMLAttributes<HTMLElement> {
  media?: SanityMediaProps
}

const ImageZoom: FC<ImageZoomProps> = ({ media, className, children }) => {
  const slidesRef = useRef(null)

  useEffect(() => {
    if (
      !slidesRef.current ||
      !(media?.image?.asset as any).path ||
  ...
  return (
    <div ref={slidesRef} className={className}>
      <div className="zoom-wrapper">
        <a
          href={`https://cdn.sanity.io/${(media?.image?.asset as any).path}`}
          data-pswp-width={1000}
          data-pswp-height={1100}
          className={classNames('cursor-zoom-in overflow-hidden')}
        >
          {children}
        </a>
      </div>
