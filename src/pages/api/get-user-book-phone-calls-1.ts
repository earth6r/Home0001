...
// curl -X GET http://localhost:3000/api/get-user-book-phone-calls?email=apinanapinan@icloud.com
export default async function handler(
  ...
  const usersBookPhoneCalls = await db
    .collection('usersBookPhoneCall1')
    .where('userUID', '==', user.docs[0].id)
    .get()

  if (usersBookPhoneCalls.empty) {
    res.status(200).json({
  ...
  const usersBookPhoneCallsValue = usersBookPhoneCalls.docs.map(doc =>
    doc.data()
  ) // Get the user's buying progress

  res.status(200).json({
    usersBookPhoneCalls: usersBookPhoneCallsValue,
  })
}
