...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (
    ...
    res.status(400).json({
      error: 'buyingProgress must be one of escrow-deposit, download-documents, schedule-closing, full-payment, or completed', // Respond with error if buyingProgress is invalid
    })
    return
  }
