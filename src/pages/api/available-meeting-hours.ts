...
async function getAvailableSlotsForDay(auth: any, date: Date): Promise<{ start: string }[]> {
  const calendar = google.calendar({ version: 'v3', auth });
  ...
  const isReserved = await getAllDayEvents(auth, dayStart.toISOString(), dayEnd.toISOString());
  if (isReserved) {
    return [];
  }

  const timeMin = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), 10, 0, 0, 0));
  const timeMax =new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), 15, 0, 0, 0));
  ...
  if (events.length === 0) {
    for (let start = timeMin; start <= timeMax; start = new Date(start.getTime() + slotOverlap)) { 
